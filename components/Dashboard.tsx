      ...
      <div className="flex flex-col gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
             <h3 className="font-bold text-lg text-slate-800">Chi tiết công tác nhân viên</h3>
          ...
          <div className="overflow-x-auto scrollbar-thin">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className="sticky left-0 z-10 bg-slate-50 px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider border-r">
                    Nhân viên
                  </th>
                  {monthsInYear.map(m => (
                    <th key={m.toString()} className="px-2 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider w-12 border-r">
                      T{getMonth(m) + 1}
                    </th>
                  ))}
                  <th className="px-4 py-4 text-center text-xs font-bold text-red-500 uppercase tracking-wider w-20 border-r bg-red-50/50">Lễ</th>
                  <th className="px-4 py-4 text-center text-xs font-bold text-medical-700 uppercase tracking-wider w-20 bg-medical-50/50">Tổng</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-slate-100">
                {detailedStats.map((staff, idx) => (
                  <tr key={staff.id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="sticky left-0 z-10 bg-white group-hover:bg-blue-50/30 px-6 py-3 whitespace-nowrap border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                      <div className="flex items-center">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white mr-3 ${['bg-blue-500', 'bg-emerald-500', 'bg-purple-500', 'bg-orange-500'][idx % 4]}`}>
                          {staff.name.charAt(0)}
                        </div>
                        <div>
                    ...
                    {staff.monthlyStats.map((stat) => (
                      <td 
                        key={stat.month} 
                        className="px-2 py-3 whitespace-nowrap text-center border-r relative"
                      >
                         {stat.count > 0 ? (
                           <span className="inline-flex items-center justify-center w-6 h-6 rounded bg-slate-100 text-xs font-bold text-slate-700 group-hover:bg-white group-hover:shadow-sm transition-all" title={`Ngày trực: ${stat.days}`}>
                             {stat.count}
                           </span>
                         ) : (
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-center border-r bg-red-50/20">
                      <span className={`text-sm font-bold ${staff.holidayShiftsCount > 0 ? 'text-red-600' : 'text-slate-300'}`}>
                        {staff.holidayShiftsCount}
                      </span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-center bg-medical-50/20">
                       <span className="inline-block px-2 py-1 rounded-md bg-medical-100 text-medical-800 text-sm font-bold">
